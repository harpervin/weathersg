...
if __name__ == "__main__":
    ...
    # Plot 1: Average Weather per location
    plt.figure(figsize=(12, 6))
    bars = plt.bar(zone_locations, zone_avg_values, color=zone_colors)

    plt.xlabel("Locations")
    plt.ylabel(f"Average {data_type.capitalize()} (deg C)")
    plt.title(
    ...
    # Plot 2: Average temp per region
    region_averages = {}
    for zone_name, district_locations in district_map.items():
        total_weather_reading = 0.0
        count = 0
        for loc in district_locations:
            if loc in location_to_avg_value:
                total_weather_reading += location_to_avg_value[loc]
                count += 1
        region_averages[zone_name] = total_weather_reading / count if count > 0 else 0
    ...
    plt.xlabel("Regions")
    plt.ylabel(f"Average {data_type} (deg C)")
    plt.title(f"Average {data_type} by Region\n({start_date.date()} to {end_date.date()})")
